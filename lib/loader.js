...
//    instead. Defaults to 'help'.
//  * manuals - If set, this location will as a repository of named manual files
//    to be used for function-only Command modules.
//
function CommandLoader(obj) {
  ...
  this.fallback = obj.fallback || 'help';
...
function run(argv) {
  ...
  var name = self.fallback;
  var command = null;

  if (!options.help && options._.length) {
    name = options._[0];
    command = self.getRun(name);
  }

  if (command) {
    argv = argv.slice(argv.indexOf(name) + 1);
    options = self.parse(argv);
  } else if (self.strict) {
    return self.error(util.format('"%s" is not an slnode command. See `slnode help` for more information.', name));
  } else {
...
function loadCommand(name) {
  ...
  try {
    module = require(path.resolve(this.root, name));
  } catch (e) {
