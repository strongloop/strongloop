...
module.exports = function(parent, done) {
  ...
  return parent.command('*')
    ...
    /*
    var config = {};
    if (cmd.allowUnsupported) {
      config['allow-unsupported'] = true;
    }
    ...
    npm.load(config, function(err) {
      ...
      if(args.length > 0) {
        var cmd = npm.commands[args[0]];
        if(cmd) {
          args.shift();
          cmd(args, function(err, data) {
            if (err) {
              return done(err);
            }
            done();
          ...
          done(new Error('unknown npm command: ' + args[0]));
        }
      }
    });
    */
