...
function strongops(argv, options) {
  ...
  var overrides2 = JSON.parse(JSON.stringify(overrides));
  var defaults2 = JSON.parse(JSON.stringify(defaults));

  // get the name and email only for login
  promptUserForLogin(overrides2, defaults2, function(err, userEnteredData) {
    overrides.email = userEnteredData.email;
    strongOpsLogin(userEnteredData, options, function(err, userData) {
      if (err) {
        if (err === '[Error: Invalid Credentials]') {
          console.log('\n\'%s\' is not registered with strongops.',
                      userEnteredData.email);
          console.log('Please answer the following to register with '+
                      'strongops:\n');
          doUserReg(overrides, defaults, options);
        } else {
        ...
        if (userData && userData.name)
          console.log('Welcome back, %s.', userData.name);
 ...
 * @param {Object} options Object with command line options.
 */
function doUserReg(overrides, defaults, options) {
  // get the registration information from the user, if we don't already
  // have it
