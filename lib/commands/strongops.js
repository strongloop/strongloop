/**
 * @fileOverview
 * This file contains the code for the slc cli command 'strongops'. The main function is
 * to register users with the strongops (née NodeFly) platform in a simple, low-friction
 * (for the user) manner.
 */
...
//module.exports = exports.run = strongops;  // for slc's loader.js to execute the code
exports.run = strongops;  // for slc's loader.js to execute the code
exports.test = {};        // for testing

/**
 * The strongops command provides a means for developers to register for a strongops account.
 * The command prompts users for the name, email and password with defaults coming from the 
 * ~/.gitconfig and ~/.npmrc. Additionally, the command take switches to skip prompting, e.g.:
 * $ slc strongops --name "Edmond Meinfelder" --email "elm@stdarg.com" --password "12345678"
  ...
  promptUserForData(overrides, defaults, function(err, userEnteredData) {
    ...
    strongOpsRegister(userEnteredData, options, function(err, userData) {
      ...
      handleCredentials(userData, options, function(err) {
        ...
        // if we have not saved the strongops credentials, echo them out to stdout
        // if they have been saved, inform the user where.
        if (timesSaved === 0)
          displayCredentials(userData);
        else if (timesSaved > 0)
          console.log('Your StrongOps credentials were written to: %s', whereSaved);
...
function strongOpsRegister(userEnteredData, options, cb) {
  strongopsReg.register(userEnteredData, function(err, userData) {
    var errStr;
    if (err)
      return cb('There was an error registering for StrongOps: '+JSON.stringify(err));

    if (!is.obj(userData))
      return cb('Error on strongops registration',userData);

    if (Object.keys(userData).length === 1 && is.nonEmptyStr(userData.message))
      return cb('There was an error on registration: '+JSON.stringify(userData.message));
    ...
    if (!is.nonEmptyObj(userData) || !is.nonEmptyStr(userData.userKey))
      return cb('There was an error. The server failed to return the user key.\n',
...
/**
 * handleCredentials checks the options and unless, options.nosave exists with a
 * truthy value, persists the user data depending on one fo teh following options:
 * global, local or package.
 * @param {Object} userData an object from the StrongOps platform with the user
 * data including the API key.
 * @param {Object} options An object containing the command line argument options
 */
...
function createEmptyJsonFileIfNone(file, cb) {
  if (!is.nonEmptyStr(file))
    return cb('createEmptyJsonFile expected a filename, received: '+JSON.stringify(file));
...
function persistUserData(userData, options, cb) {
  ...
  if (!options.saveall && !options.local && !options.package && !options.global) return cb();
...
/**
 * Using the prompt module, interactively query users for name, email and a password to use as
 * credentials for the strongops platform.
...
function promptUserForData(overrides, defaults, cb) {
  ...
  var result;     // storage from prompt results, so different parts of asynch.whilst can use
  var count = 0;  // we limit the number of re-tries for entering a password to 3
  ...
  async.doWhilst(
    ...
    function() {
      ...
      // handle edgecase where command line can be a number when we need a string
      if (is.number(result.password)) result.password = result.password.toString();
      if (is.number(result.password2)) result.password2 = result.password2.toString();

      // do the password and its confirmation match?
      if (result.password !== result.password2) {
        // if the user can't get it in 3 tries, give up
        if (++count > 3) {
          console.error('Giving up after 3 re-tries to enter a password and matching confirmation.');
          return false;
        }
        if (result.password) delete result.password;
        if (result.password2) delete result.password2;
        prompt.override = result;
        console.error('The password does not match its confirmation. Please try again.');
        return true;
      }
      return false;
    },
 ...
 * @returns {String|Undefined} The email address from .npmrc or undefined, if none
 */
...
/**
 * Read a file into a buffer using fs.readFileSync will
 * catch in eth case the file does not exist and returns
 * false when there is no file (or it can't be read) and
 * a string with the file contents otherwise.
 * @param {String} filename a file name with path information.
 * @returns {Boolean|String} false if the file can't be read or
 * a buffer with the file contents otherwise
 */
