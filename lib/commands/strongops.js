...
// This code is radically different in style, and intended to be deleted soon.
/* eslint-disable */
 ...
 */
function strongops(argv, options, loader) {
  if (options.help || options.h) {
    return loader.printUsage('strongops');
  }
  ...
  function handleResponseCb(err, userEnteredData) {
    if (err) {
      displayError(getErrorForUser(err));
      process.exit(1);
    } else{
      // success
      return finished(userEnteredData);
    }
  }

  function finished(userData) {
    if (!is.nonEmptyObj(userData)) {
      displayError('Authenticated, but user data is invalid: '+
 ...
 * @param {Function} cb A callback function(err, bool)
 */
function promptToLoginOrRegister(cb) {

  console.log('\nWould you like to:');
  prompt.message = '(l)ogin with different credentials or (q)uit?';

  var schema = {
    properties: {
      answer: {
        pattern: /^[lqLQ]$/,
        message: 'Response can be either \'l\' or \'q\'.',
        default: 'l',
        required: true
      },
    }
...
function promptUserForLogin(overrides, defaults, cb) {
  console.log([
    'Generating strongops configuration requires a registered email and',
    'password.',
  ...
  var promptSchema = {
    properties: {
      ...
      password: {
        description: 'password',
        pattern: /^.+$/,
        message: 'A password must be at least 1 character.',
        hidden: true,
        required: true
      }
    }
