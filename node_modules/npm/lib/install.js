...
function checkSupported(target, cb) {
  if (target.supported || npm.config.get('allow-unsupported'))
    return cb();

  var msg = format("%s@%s is not supported by StrongLoop.\n" +
                   "If you wish to install this package anyway, you can use\n" +
                   "the `allow-unsupported` option to override this check.",
                   target.name,
                   target.version);
